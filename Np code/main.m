...
if isempty(p)
  ...
  % pump_rates=[0.2,0.4,0.5,0.9,1,1.2,1.5,2.];%[1,2,5,10];%[0.1:0.1:1,2:1:5,10:5:25,50:25:100];
% pump_rates=1;
% NNp=[1,5,10,25,50];
NNp=[1,4,6,8,12];
pump_rates=10*[0.1,0.2,0.4,0.5,1,1.5,2.0];


 for ind2=1:length(pump_rates)

for ind=1:length(NNp)
...
Gamma_p=W*zeros(N);%can be correlated/local/collective spin pump
Gamma_p(1:Np,1:Np)=W;
...
if DO_PARALLEL
    for i = 1:Nrep 
        solver_spins_corr_emission_real_loop_par(J_l,Gamma_l,V_l,mask_l,J_p,Gamma_p,V_p,mask_p,Eps,i,Np)
    end
else
...
tic;
loop_read_cluster(J_l,Gamma_l,V_l,mask_l,J_p,Gamma_p,V_p,mask_p,NNp);
toc;
end
 end
